...
/** Live handle for the current page session (avoids re-reading IDB on every op). */
let activeRoot: FSDirHandle | null = null;
...
export async function saveRootHandle(handle: FSDirHandle): Promise<void> {
  ...
  try {
    ...
    // Keep the in-memory handle even if IDB persistence fails (e.g. private mode).
...
async function requireRoot(): Promise<FSDirHandle> {
  ...
  async hasPermissionGranted(): Promise<boolean> {
    ...
    if (root.queryPermission) {
      try {
        ...
        console.error("[workspace] queryPermission failed:", err);
        return false;
      }
    }
    // No queryPermission API — treat stored handle as needing an explicit reconnect click.
    return false;
  },
  ...
  async requestPermissionIfNeeded(): Promise<boolean> {
    ...
    try {
      ...
      if (root.requestPermission) {
        ...
        if (state === "granted") {
          activeRoot = root;
          // Re-persist so the handle stays durable after a successful re-grant.
      ...
      console.error("[workspace] requestPermission failed:", err);
      return false;
    }
    return false;
  },
