...
// Batched projection on the model backend. X and Y use token-major host
// layout, which is identical to ggml tensors [cols,T] and [rows,T]. This is
// especially important in WebAssembly: looping over prompt positions with
// CPU matvecs can take tens of seconds even though the transformer itself is
// already resident on WebGPU.
static bool matmul_backend_graph(voxcpm2_context* ctx, ggml_tensor* W, ggml_tensor* b, const float* X, int cols,
                                 int rows, int T, float* Y, const char* tag) {
    if (!ctx || !ctx->backend || !W || !X || !Y || T <= 0) {
        return false;
    }
    ...
    ggml_tensor* input = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, cols, T);
    ggml_set_name(input, "projection_input");
    ggml_set_input(input);
    ggml_tensor* output = ggml_mul_mat(ctx0, W, input);
    if (b) {
        output = ggml_add(ctx0, output, b);
    }
    ggml_set_name(output, "projection_output");
    ggml_set_output(output);
    ggml_build_forward_expand(gf, output);
    ggml_free(ctx0);

    if (!ggml_gallocr_alloc_graph(ctx->galloc, gf)) {
        fprintf(stderr, "voxcpm2: %s projection graph allocation failed\n", tag);
        return false;
    }
    ggml_backend_tensor_set(input, X, 0, (size_t)cols * T * sizeof(float));
    if (ggml_backend_graph_compute(ctx->backend, gf) != GGML_STATUS_SUCCESS) {
        fprintf(stderr, "voxcpm2: %s projection graph compute failed\n", tag);
        return false;
    }
    ggml_backend_tensor_get(output, Y, 0, (size_t)rows * T * sizeof(float));
    return true;
}
...
static ggml_cgraph* build_ralm_step_graph(voxcpm2_context* ctx, int n_past, int n_tokens = 1) {
    const vox_hparams& hp = ctx->hp;
    ...
    const int T = std::max(1, n_tokens);
    ...
    // A single decode token can see the complete populated cache. Batched
    // prefill needs a lower-triangular mask so prompt positions cannot attend
    // to future prompt positions.
...
static ggml_cgraph* build_tslm_step_graph(voxcpm2_context* ctx, int n_past, int fixed_kv_len = 0,
    ...
    if (T == 1 && W.stop_proj_w && W.stop_proj_b && W.stop_head_w && W.fsq_in_proj_w && W.fsq_out_proj_w) {
        // FSQ path
    ...
    } else if (T == 1 && W.stop_proj_w && W.stop_proj_b && W.stop_head_w) {
...
// GPU-native prefill for the browser and other discrete backends. The legacy
// prefill walks every transformer layer with tiny CPU matvec graphs, which is
// prohibitively slow in WebAssembly and makes the first synthesis appear hung
// before AR step 0. Reusing the backend step graphs keeps transformer prefill
// and KV population on WebGPU.
static bool tslm_prefill_from_embeds_graph(voxcpm2_context* ctx, const float* embeds, int T,
                                           std::vector<float>& normalized_all) {
    const int d = (int)ctx->hp.tslm_d_model;
    if (ctx->verbosity >= 1) {
        fprintf(stderr, "voxcpm2: WebGPU TSLM prefill starting (%d positions)\n", T);
    }
    ctx->tslm_kv.reset();
    ...
    normalized_all.assign((size_t)T * d, 0.0f);

    // Submit the full prompt as one causal graph. The former per-position
    // loop submitted all 28 transformer layers T times and dominated browser
    // latency even for a short Khmer greeting.
    ggml_cgraph* gf = build_tslm_step_graph(ctx, /*n_past=*/0, /*fixed_kv_len=*/0,
                                            /*arena_ctx=*/nullptr, /*n_tokens=*/T);
    if (!gf || !ggml_gallocr_alloc_graph(ctx->galloc, gf)) {
        fprintf(stderr, "voxcpm2: TSLM prefill graph allocation failed\n");
        return false;
    }
    ...
    ggml_tensor* t_positions = ggml_graph_get_tensor(gf, "positions");
    ggml_tensor* t_mask = ggml_graph_get_tensor(gf, "causal_mask");
    if (!t_hidden_in || !t_positions || !t_mask) {
        fprintf(stderr, "voxcpm2: TSLM prefill graph inputs are incomplete\n");
        return false;
    }
    ggml_backend_tensor_set(t_hidden_in, embeds, 0, (size_t)T * d * sizeof(float));
    ...
    const ggml_fp16_t ninf = ggml_fp32_to_fp16(-INFINITY);
    for (int q = 0; q < T; q++) {
        positions[(size_t)q] = q;
        for (int k = 0; k < T; k++) {
            mask[(size_t)q * T + k] = (k <= q) ? z : ninf;
        }
    }
    ggml_backend_tensor_set(t_positions, positions.data(), 0, positions.size() * sizeof(int32_t));
    ggml_backend_tensor_set(t_mask, mask.data(), 0, mask.size() * sizeof(ggml_fp16_t));

    if (ggml_backend_graph_compute(ctx->backend, gf) != GGML_STATUS_SUCCESS) {
        fprintf(stderr, "voxcpm2: TSLM prefill graph compute failed\n");
        return false;
    }
    ggml_tensor* out = ggml_graph_get_tensor(gf, "hidden_out");
    if (!out) {
        fprintf(stderr, "voxcpm2: TSLM prefill graph output is missing\n");
        return false;
    }
    ggml_backend_tensor_get(out, normalized_all.data(), 0, normalized_all.size() * sizeof(float));
    ctx->tslm_kv.n_past = T;
    if (ctx->verbosity >= 1) {
        fprintf(stderr, "voxcpm2: WebGPU TSLM prefill %d/%d (batched)\n", T, T);
    }
    return true;
}

// WebGPU currently lacks a fused flash-attention shader for several VoxCPM2
// shapes. Fall back to the mathematically equivalent matmul + softmax path on
// that backend while retaining fused attention everywhere else.
static ggml_tensor* vox_graph_attention(ggml_context* ctx0, ggml_backend_t backend, ggml_tensor* Q,
                                        ggml_tensor* K, ggml_tensor* V, ggml_tensor* mask, float scale) {
    const char* name = backend ? ggml_backend_name(backend) : "";
    if (name && std::strncmp(name, "WebGPU", 6) == 0) {
        ...
        return ggml_mul_mat(ctx0, Vt, scores);
    }
    return ggml_flash_attn_ext(ctx0, Q, K, V, mask, scale, /*max_bias*/ 0.0f,
                               /*logit_softcap*/ 0.0f);
}

static bool ralm_prefill_multi_graph(voxcpm2_context* ctx, const float* input, int T,
                                     std::vector<float>& normalized_all) {
    const int d = (int)ctx->hp.ralm_d_model;
    ctx->ralm_kv.reset();
    if (!init_ralm_kv_backend(ctx)) {
    ...
    ggml_cgraph* gf = build_ralm_step_graph(ctx, /*n_past=*/0, /*n_tokens=*/T);
    if (!gf || !ggml_gallocr_alloc_graph(ctx->galloc, gf)) {
        fprintf(stderr, "voxcpm2: RALM prefill graph allocation failed\n");
        return false;
    }

    ggml_tensor* t_hidden_in = ggml_graph_get_tensor(gf, "ralm_hidden_in");
    ggml_tensor* t_mask = ggml_graph_get_tensor(gf, "ralm_causal_mask");
    if (!t_hidden_in || !t_mask) {
        fprintf(stderr, "voxcpm2: RALM prefill graph inputs are incomplete\n");
        return false;
    }
    ggml_backend_tensor_set(t_hidden_in, input, 0, (size_t)T * d * sizeof(float));
    ...
    if (ggml_backend_graph_compute(ctx->backend, gf) != GGML_STATUS_SUCCESS) {
        fprintf(stderr, "voxcpm2: RALM prefill graph compute failed\n");
        return false;
    }
    ggml_tensor* out = ggml_graph_get_tensor(gf, "ralm_hidden_out");
    if (!out) {
        fprintf(stderr, "voxcpm2: RALM prefill graph output is missing\n");
        return false;
    }
    ggml_backend_tensor_get(out, normalized_all.data(), 0, normalized_all.size() * sizeof(float));
    ctx->ralm_kv.n_past = T;
    if (ctx->verbosity >= 1) {
        fprintf(stderr, "voxcpm2: WebGPU RALM prefill %d/%d (batched)\n", T, T);
    }
    return true;
}
...
static std::vector<float> fsq_forward_graph(voxcpm2_context* ctx, const float* x) {
    const vox_weights& W = ctx->graph_weights();
    constexpr int d_in = 2048;
    constexpr int d_mid = 512;
    ...
    ggml_cgraph* gf = ggml_new_graph_custom(ctx0, 64, false);
    ggml_tensor* input = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, d_in, 1);
    ggml_set_name(input, "fsq_input");
    ggml_set_input(input);
    ggml_tensor* half = ggml_new_tensor_1d(ctx0, GGML_TYPE_F32, d_mid);
    ggml_set_name(half, "fsq_half_input");
    ggml_set_input(half);

    ggml_tensor* mid = ggml_mul_mat(ctx0, W.fsq_in_proj_w, input);
    if (W.fsq_in_proj_b) {
        mid = ggml_add(ctx0, mid, W.fsq_in_proj_b);
    }
    mid = ggml_tanh(ctx0, mid);
    mid = ggml_scale(ctx0, mid, 9.0f);
    mid = ggml_add(ctx0, mid, half);
    mid = ggml_floor(ctx0, mid);
    mid = ggml_scale(ctx0, mid, 1.0f / 9.0f);
    ggml_tensor* output = ggml_mul_mat(ctx0, W.fsq_out_proj_w, mid);
    if (W.fsq_out_proj_b) {
        output = ggml_add(ctx0, output, W.fsq_out_proj_b);
    }
    ggml_set_name(output, "fsq_output");
    ggml_set_output(output);
    ggml_build_forward_expand(gf, output);
    ggml_free(ctx0);

    if (!ggml_gallocr_alloc_graph(ctx->galloc, gf)) {
        fprintf(stderr, "voxcpm2: FSQ graph allocation failed\n");
        return {};
    }
    ggml_backend_tensor_set(input, x, 0, (size_t)d_in * sizeof(float));
    std::vector<float> half_values(d_mid, 0.5f);
    ggml_backend_tensor_set(half, half_values.data(), 0, half_values.size() * sizeof(float));
    if (ggml_backend_graph_compute(ctx->backend, gf) != GGML_STATUS_SUCCESS) {
        fprintf(stderr, "voxcpm2: FSQ graph compute failed\n");
        return {};
    }
    std::vector<float> result(d_out);
...
static ggml_cgraph* build_locenc_graph(voxcpm2_context* ctx, ggml_context* arena_ctx = nullptr) {
    ...
    for (uint32_t il = 0; il < hp.locenc_n_layers; il++) {
        ...
        ggml_tensor* attn = vox_graph_attention(ctx0, ctx->backend, Q, K, V, /*mask=*/nullptr, ascale);
...
static float stop_score_graph(voxcpm2_context* ctx, const float* lm_hidden) {
    const vox_weights& W = ctx->graph_weights();
    const int d_lm = (int)ctx->hp.tslm_d_model;
    ...
    ggml_cgraph* gf = ggml_new_graph_custom(ctx0, 32, false);
    ggml_tensor* input = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, d_lm, 1);
    ggml_set_name(input, "stop_input");
    ggml_set_input(input);
    ggml_tensor* hidden = ggml_mul_mat(ctx0, W.stop_proj_w, input);
    hidden = ggml_add(ctx0, hidden, W.stop_proj_b);
    hidden = ggml_silu(ctx0, hidden);
    ggml_tensor* probs = ggml_mul_mat(ctx0, W.stop_head_w, hidden);
    probs = ggml_soft_max(ctx0, probs);
    ggml_set_name(probs, "stop_output");
    ggml_set_output(probs);
    ggml_build_forward_expand(gf, probs);
    ggml_free(ctx0);

    if (!ggml_gallocr_alloc_graph(ctx->galloc, gf)) {
        fprintf(stderr, "voxcpm2: stop predictor graph allocation failed\n");
        return -1.0f;
    }
    ggml_backend_tensor_set(input, lm_hidden, 0, (size_t)d_lm * sizeof(float));
    if (ggml_backend_graph_compute(ctx->backend, gf) != GGML_STATUS_SUCCESS) {
        fprintf(stderr, "voxcpm2: stop predictor graph compute failed\n");
        return -1.0f;
    }
    float output[2] = {0.0f, 0.0f};
    ggml_backend_tensor_get(probs, output, 0, sizeof(output));
    return std::isnan(output[1]) ? -1.0f : output[1];
}
    ...
    *out_n_samples = 0;
    ...
    if (graph_prefill) {
        if (!tslm_prefill_from_embeds_graph(ctx, pi.combined_embed.data(), N_pos, all_pos)) {
            fprintf(stderr, "voxcpm2: WebGPU TSLM prefill failed\n");
            return nullptr;
        }
    } else {
    ...
    if (graph_prefill) {
        // tslm_step_graph folds the final RMSNorm into its output.
        normed_all = std::move(all_pos);
    } else {
    ...
    {
        int in_dim = 2 * d_tslm;
        std::vector<float> fusion_input((size_t)N_pos * in_dim, 0.0f);
        std::vector<float> ralm_input((size_t)N_pos * d_tslm);
        for (int i = 0; i < N_pos; i++) {
            float* cat_buf = fusion_input.data() + (size_t)i * in_dim;
            std::memcpy(cat_buf, normed_all.data() + (size_t)i * d_tslm, (size_t)d_tslm * sizeof(float));
            if (audio_mask_pos[i]) {
                std::memcpy(cat_buf + d_tslm, feat_embed_pos.data() + (size_t)i * d_tslm,
                            (size_t)d_tslm * sizeof(float));
            }
        }
        const vox_weights& graph_w = ctx->graph_weights();
        if (!graph_prefill ||
            !matmul_backend_graph(ctx, graph_w.fusion_w, graph_w.fusion_b, fusion_input.data(), in_dim, d_tslm,
                                  N_pos, ralm_input.data(), "fusion prefill")) {
            for (int i = 0; i < N_pos; i++) {
                matmul_mv_bias(cpu_be, ctx->weights.fusion_w, ctx->weights.fusion_b,
                               fusion_input.data() + (size_t)i * in_dim, in_dim,
                               ralm_input.data() + (size_t)i * d_tslm, d_tslm);
            }
        }
        std::vector<float> ralm_out;
        if (graph_prefill) {
            if (!ralm_prefill_multi_graph(ctx, ralm_input.data(), N_pos, ralm_out)) {
                fprintf(stderr, "voxcpm2: WebGPU RALM prefill failed\n");
                return nullptr;
            }
        } else {
            ralm_out = ralm_prefill_multi(ctx, ralm_input.data(), N_pos, cpu_be);
        }
        int dr = (int)hp.ralm_d_model;
        ralm_hidden.resize(dr);
        if (graph_prefill) {
            // ralm_step_graph also returns the final normalized state.
            std::memcpy(ralm_hidden.data(), ralm_out.data() + (size_t)(N_pos - 1) * dr,
                        (size_t)dr * sizeof(float));
        } else {
    ...
    auto build_mu = [&](const std::vector<float>& lm_h, const std::vector<float>& ralm_h) -> std::vector<float> {
        ...
        if (ctx->weights.lm_to_dit_w && ctx->weights.lm_to_dit_b) {
            if (!graph_prefill ||
                !matmul_backend_graph(ctx, graph_w.lm_to_dit_w, graph_w.lm_to_dit_b, lm_h.data(), d_lm, d_dit, 1,
                                      mu.data(), "lm_to_dit")) {
                matmul_mv_bias(cpu_be, ctx->weights.lm_to_dit_w, ctx->weights.lm_to_dit_b, lm_h.data(), d_lm,
                               mu.data(), d_dit);
            }
        }
        ...
        if (ctx->weights.res_to_dit_w && ctx->weights.res_to_dit_b) {
            if (!graph_prefill ||
                !matmul_backend_graph(ctx, graph_w.res_to_dit_w, graph_w.res_to_dit_b, ralm_h.data(), d_ralm,
                                      d_dit, 1, mu.data() + d_dit, "res_to_dit")) {
    ...
    ctx->tslm_kv_synced = graph_prefill;
    ctx->ralm_kv_synced = graph_prefill;
    ...
    while (step < effective_max) {
        ...
        if (ctx->weights.enc_to_lm_w && ctx->weights.enc_to_lm_b) {
            const vox_weights& graph_w = ctx->graph_weights();
            if (!graph_prefill ||
                !matmul_backend_graph(ctx, graph_w.enc_to_lm_w, graph_w.enc_to_lm_b, enc_out.data(), d_dit, d_lm, 1,
                                      enc_lm.data(), "enc_to_lm")) {
                matmul_mv_bias(cpu_be, ctx->weights.enc_to_lm_w, ctx->weights.enc_to_lm_b, enc_out.data(), d_dit,
                               enc_lm.data(), d_lm);
            }
        } else {
        ...
        // fall through to the CPU stop_score as before.
        {
            ...
            float sp = have_graph_stop ? graph_stop_score
                                       : (graph_prefill ? stop_score_graph(ctx, tslm_hidden.data()) : -1.0f);
            if (sp < 0.0f) {
                sp = stop_score(ctx, tslm_hidden.data(), cpu_be);
            }
            if (bench)
        ...
        // 3b. FSQ
        tb = bench ? vox_now_ms() : 0;
        fsq_out = graph_prefill ? fsq_forward_graph(ctx, tslm_hidden.data())
                                : fsq_forward(ctx, tslm_hidden.data(), cpu_be);
        if (fsq_out.empty()) {
        ...
        if (ctx->weights.fusion_w && ctx->weights.fusion_b) {
            const vox_weights& graph_w = ctx->graph_weights();
            if (!graph_prefill ||
                !matmul_backend_graph(ctx, graph_w.fusion_w, graph_w.fusion_b, fusion_in.data(), 2 * d_lm, d_ralm, 1,
                                      fusion_out.data(), "fusion step")) {
...
void voxcpm2_set_inference_steps(struct voxcpm2_context* ctx, int steps) {
    if (ctx)
        ctx->inference_steps = std::max(1, steps);
}

void voxcpm2_set_cfg_value(struct voxcpm2_context* ctx, float cfg_value) {
    if (ctx)
        ctx->cfg_value = std::max(0.0f, cfg_value);
}

void voxcpm2_set_max_len(struct voxcpm2_context* ctx, int max_len) {
    if (ctx)
        ctx->max_len = std::max(4, max_len);
}
