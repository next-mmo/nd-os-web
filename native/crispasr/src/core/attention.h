...
static inline ggml_tensor* kv_self_attn(ggml_context* ctx0, ggml_cgraph* gf, ggml_tensor* x, ggml_tensor* q_w,
    ...
    // ---- Attention + reshape + output projection ----
    // ggml-webgpu's fused flash-attention shader does not cover every
    // head/context combination used by VoxCPM2 and aborts when no shader path
    // is selected. Its primitive matmul/softmax kernels do cover this shape,
    // so use the standard attention decomposition on WebGPU.
    const char* kv_buft_name = kv_k->buffer ? ggml_backend_buft_name(ggml_backend_buffer_get_type(kv_k->buffer)) : nullptr;
    const bool use_standard_webgpu_attention = kv_buft_name && std::strstr(kv_buft_name, "WebGPU") != nullptr;
    ggml_tensor* attn = nullptr;
    if (use_standard_webgpu_attention) {
        ggml_tensor* scores = ggml_mul_mat(ctx0, Kfull, Q); // [Lk, T, n_heads]
        scores = ggml_scale(ctx0, scores, p.attn_scale);
        if (causal_mask) {
            scores = ggml_add(ctx0, scores, causal_mask);
        }
        scores = ggml_soft_max(ctx0, scores);
        ggml_tensor* Vt = ggml_cont(ctx0, ggml_transpose(ctx0, Vfull)); // [Lk, hd, n_heads]
        attn = ggml_mul_mat(ctx0, Vt, scores);                           // [hd, T, n_heads]
    } else {
