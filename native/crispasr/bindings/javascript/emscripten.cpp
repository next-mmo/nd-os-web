...
void crispasr_set_gpu_backend(const char* name);
...
// Mirrors crispasr_open_params_v1 from the C ABI. Keep this private binding
// copy so browser callers can request native progress logs without exposing a
// raw pointer or changing the public C header.
struct crispasr_js_open_params_v2 {
    int abi_version;
    int n_threads;
    int use_gpu;
    int verbosity;
...
EMSCRIPTEN_BINDINGS(whisper) {
    ...
    // Web workers use this variant to surface native stage checkpoints in the
    // UI. Flash attention is disabled because ggml-webgpu does not implement
    // every VoxCPM2 head/context shape; the model supplies an equivalent
    // matmul + softmax attention graph for WebGPU.
    emscripten::function(
        "ttsOpenExplicitVerbose",
        emscripten::optional_override([](const std::string& model_path, const std::string& backend, int n_threads,
                                         int verbosity) {
            if (g_tts_session != nullptr) {
                crispasr_session_close(g_tts_session);
            ...
            crispasr_js_open_params_v2 params = {};
            params.abi_version = 2;
            params.n_threads = n_threads <= 0 ? 1 : n_threads;
            params.use_gpu = 1;
            params.verbosity = verbosity > 0 ? verbosity : 0;
            params.flash_attn = 0;
            params.n_gpu_layers = -1;
            const char* backend_name = backend.empty() ? nullptr : backend.c_str();
            g_tts_session = crispasr_session_open_with_params(model_path.c_str(), backend_name, &params);
            return g_tts_session != nullptr;
        }),
        emscripten::async());
    ...
    // --- Available backends ---
    emscripten::function("setGpuBackend", emscripten::optional_override([](const std::string& backend) {
                             crispasr_set_gpu_backend(backend.c_str());
                         }));
